...
│
├── database/
│   └── bankomunal.sql         # Script de creación del esquema (4 tablas)
│
├── .gitignore
└── README.md
```
...
backup.mysqldump-path / backup.mysql-path = ruta de tus binarios de MySQL (necesario para el módulo de respaldo)
spring.mail.username / spring.mail.password = credenciales de un correo Gmail con contraseña de aplicación (opcional, usado en recuperación de contraseña)
```
...
Abre la carpeta `frontend/` con Live Server (puerto recomendado `5500`) o cualquier servidor estático. Luego entra a `login.html` o `index.html`.
...
> El CORS del backend ya permite `http://localhost:5500` y `http://127.0.0.1:5500`.
...
- **Cuentas y transferencias:** consulta de saldo, transferencias entre socios, pago de servicios, historial de movimientos y comprobantes reales.
...
- **Préstamos solidarios:** (solicitud, pagos, gestión)
...
- **Interfaz responsive:** e internacionalización (ES/EN)
- **Comunidad:** creación de grupos de ahorro, fondo común, muro de publicaciones, eventos y reuniones con actas.
